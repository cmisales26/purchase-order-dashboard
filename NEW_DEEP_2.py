...
def create_invoice_pdf(invoice_data, logo_file="logo_final.jpg", stamp_file="stamp.jpg"):
    ...
    for i, item in enumerate(invoice_data["items"], start=1):
        ...
        pdf.multi_cell(col_widths[1], line_height, item['description'], border="LRTB", align="L")
        ...
        pdf.multi_cell(col_widths[0], row_height, str(i), border="LRTB", align="C")
        
        pdf.set_xy(x_start + col_widths[0] + col_widths[1], y_start)
        pdf.multi_cell(col_widths[2], row_height, item['hsn'], border="LRTB", align="C")
        
        pdf.set_xy(x_start + sum(col_widths[:3]), y_start)
        pdf.multi_cell(col_widths[3], row_height, str(item['quantity']), border="LRTB", align="C")
        
        pdf.set_xy(x_start + sum(col_widths[:4]), y_start)
        pdf.multi_cell(col_widths[4], row_height, f"{item['unit_rate']:.2f}", border="LRTB", align="R")
        
        amount = item['quantity'] * item['unit_rate']
        pdf.set_xy(x_start + sum(col_widths[:-1]), y_start)
        pdf.multi_cell(col_widths[5], row_height, f"{amount:.2f}", border="LRTB", align="R")
