...
def add_page_two_commercials(pdf, data):
    ...
    # Terms & Conditions (with bold labels)
    terms = [
    ...
    col1_width = page_width * 0.6  # Left: Terms
    col2_width = page_width * 0.4  # Right: Bank Details
    padding = 4
    line_height = 4.5
    section_spacing = 2

    # Calculate required height for both columns
    def calculate_column_height(items, col_width, is_terms=True):
        height = 0
        for item in items:
            if is_terms:
                label, detail = item
    ...
    terms_height = calculate_column_height(terms, col1_width, True)
    bank_height = calculate_column_height(bank_info, col2_width, False) + 35  # + space for signature
    ...
    for label, value in bank_info:
        ...
        pdf.cell(0, line_height, f"{label}: ", ln=0)
        pdf.set_font("Helvetica", "", 9)
        pdf.multi_cell(col2_width - 2*padding, line_height, value)
        bank_y = pdf.get_y()
