# import streamlit as st
# from fpdf import FPDF
# from num2words import num2words
# import datetime
# import os

# # --- Product Catalog ---
# PRODUCT_CATALOG = {
#     "Autodesk Commercial Software License": {"basic": 2000.0, "gst_percent": 18.0},
#     "Solidworks Premium": {"basic": 50000.0, "gst_percent": 18.0},
#     "Catia License": {"basic": 75000.0, "gst_percent": 18.0},
#     "Mastercam Module": {"basic": 30000.0, "gst_percent": 18.0},
#     "Siemens NX": {"basic": 65000.0, "gst_percent": 18.0},
# }

# # --- PDF Class ---
# class PDF(FPDF):
#     def __init__(self):
#         super().__init__()
#         self.set_auto_page_break(auto=False, margin=0)
#         self.set_left_margin(15)
#         self.set_right_margin(15)
#         self.logo_path = None
#         self.website_url = "https://cminfotech.com/"
#         font_dir = os.path.join(os.path.dirname(__file__), "fonts")
#         self.add_font("Calibri", "", os.path.join(font_dir, "calibri.ttf"), uni=True)
#         self.add_font("Calibri", "B", os.path.join(font_dir, "calibrib.ttf"), uni=True)
#         self.add_font("Calibri", "I", os.path.join(font_dir, "calibrii.ttf"), uni=True)
#         self.add_font("Calibri", "BI", os.path.join(font_dir, "calibriz.ttf"), uni=True)

#     def header(self):
#         if self.page_no() == 1:
#             if self.logo_path and os.path.exists(self.logo_path):
#                 self.image(self.logo_path, x=162.5, y=2.5, w=45, link=self.website_url)
#             self.set_font("Calibri", "B", 15)
#             self.cell(0, 15, pdf_type, ln=True, align="C")
#             self.ln(2)
#             self.set_font("Calibri", "", 12)
#             self.cell(95, 8, f"PO No: {st.session_state.po_number}", ln=0)
#             self.cell(95, 8, f"Date: {st.session_state.po_date}", ln=1)
#             self.ln(2)

#     def footer(self):
#         self.set_y(-18)
#         self.set_font("Calibri", "I", 10)
#         self.multi_cell(0, 4,
#             "E402, Ganesh Glory 11, Near BSNL Office, Jagatpur - Chenpur Road, Ahmedabad - 382481\n",
#             align="C"
#         )
#         self.set_text_color(0, 0, 255)
#         email1 = "cad@cmi.com"
#         email2 = "info@cminfotech.com"
#         self.cell(0, 4, f"{email1} | {email2}", ln=True, align="C", link=f"mailto:{email1}")
#         self.set_x((self.w - 80) / 2)
#         self.cell(0, 0, "", link=f"mailto:{email2}")
#         phone_number = "+91 873 391 5721"
#         self.set_x((self.w - 60) / 2)
#         self.cell(60, 4, f"Call: {phone_number}", ln=True, align="C", link=f"tel:{phone_number}")
#         self.set_text_color(0, 0, 0)

#     def section_title(self, title):
#         self.set_font("Calibri", "B", 12)
#         self.cell(0, 6, title, ln=True)
#         self.ln(1)

#     def sanitize_text(self, text):
#         return text.encode('ascii', 'ignore').decode('ascii')

# # --- Initialize Session ---
# if "po_seq" not in st.session_state:
#     st.session_state.po_seq = 1
# if "products" not in st.session_state:
#     st.session_state.products = []
# if "company_name" not in st.session_state:
#     st.session_state.company_name = "CM Infotech"

# today = datetime.date.today()
# st.session_state.po_date = today.strftime("%d-%m-%Y")
# default_po_number = f"C/CP/{today.year}/Q{(today.month-1)//3+1}/{st.session_state.get('po_seq', 1):03d}"
# st.session_state.po_number = st.text_input("PO Number", value=default_po_number)

# st.set_page_config(page_title="PO & Tax Invoice Generator", page_icon="📄", layout="wide")
# st.title("📄 PO & Tax Invoice Generator")

# with st.sidebar:
#     st.header("Company Info")
#     logo_file = st.file_uploader("Upload Company Logo", type=["png", "jpg", "jpeg"])
#     logo_path = None
#     if logo_file:
#         logo_path = "logo_temp.png"
#         with open(logo_path, "wb") as f:
#             f.write(logo_file.getbuffer())
#     auto_increment = st.checkbox("Auto-increment PO Number", value=True)
#     if st.button("Reset PO Sequence"):
#         st.session_state.po_seq = 1
#         st.success("PO sequence reset to 1")

# pdf_type = st.selectbox("Document Type", ["Purchase Order", "Tax Invoice"])

# tab1, tab2, tab3, tab4 = st.tabs(["Vendor Details", "Products", "Terms", "Preview & Generate"])

# with tab1:
#     col1, col2 = st.columns(2)
#     with col1:
#         vendor_name = st.text_input("Vendor Name", "Arkance IN Pvt. Ltd.")
#         vendor_address = st.text_area("Vendor Address", "Unit 801-802, 8th Floor, Tower 1...")
#         vendor_contact = st.text_input("Contact Person", "Ms/Mr")
#         vendor_mobile = st.text_input("Mobile", "+91 1234567890")
#         gst_no = st.text_input("GST No", "24ANMPP4891R1ZX")
#         pan_no = st.text_input("PAN No", "ANMPP4891R")
#         msme_no = st.text_input("MSME No", "UDYAM-GJ-01-0117646")
#     with col2:
#         bill_to_company = st.text_input("Bill To", "CM INFOTECH")
#         bill_to_address = st.text_area("Bill To Address", "E/402, Ganesh Glory 11, Near BSNL Office, Jagatpur Chenpur Road, Ahmedabad - 382481")
#         ship_to_company = st.text_input("Ship To", "CM INFOTECH")
#         ship_to_address = st.text_area("Ship To Address", "E/402, Ganesh Glory 11, Near BSNL Office, Jagatpur Chenpur Road, Ahmedabad - 382481")
#         end_company = st.text_input("End User Company", "Baldridge & Associates Pvt Ltd.")
#         end_address = st.text_area("End User Address", "406 Sakar East, Vadodara 390009")
#         end_person = st.text_input("End User Contact", "Mr. Dev")
#         end_contact = st.text_input("End User Phone", "+91 9876543210")
#         end_email = st.text_input("End User Email", "info@company.com")

# with tab2:
#     st.header("Products")
#     selected_product = st.selectbox("Select from Catalog", [""] + list(PRODUCT_CATALOG.keys()))
#     if st.button("➕ Add Selected Product"):
#         if selected_product:
#             details = PRODUCT_CATALOG[selected_product]
#             st.session_state.products.append({
#                 "name": selected_product,
#                 "basic": details["basic"],
#                 "gst_percent": details["gst_percent"],
#                 "qty": 1.0,
#             })
#     if st.button("➕ Add Empty Product"):
#         st.session_state.products.append({"name": "New Product", "basic": 0.0, "gst_percent": 18.0, "qty": 1.0})

#     for i, p in enumerate(st.session_state.products):
#         with st.expander(f"Product {i+1}: {p['name']}", expanded=i==0):
#             col1, col2, col3, col4 = st.columns([3,1,1,1])
#             st.session_state.products[i]["name"] = st.text_input("Name", p["name"], key=f"name_{i}")
#             st.session_state.products[i]["basic"] = st.number_input("Basic (₹)", p["basic"], format="%.2f", key=f"basic_{i}")
#             st.session_state.products[i]["gst_percent"] = st.number_input("GST %", p["gst_percent"], format="%.1f", key=f"gst_{i}")
#             st.session_state.products[i]["qty"] = st.number_input("Qty", p["qty"], format="%.2f", key=f"qty_{i}")
#             if st.button("Remove", key=f"remove_{i}"):
#                 st.session_state.products.pop(i)
#                 st.rerun()

# with tab3:
#     st.header("Terms & Authorization")
#     col1, col2 = st.columns(2)
#     with col1:
#         payment_terms = st.text_input("Payment Terms", "30 Days from Invoice date")
#         delivery_days = st.number_input("Delivery (Days)", min_value=1, value=2)
#         delivery_terms = st.text_input("Delivery Terms", f"Within {delivery_days} Days")
#     with col2:
#         prepared_by = st.text_input("Prepared By", "Finance Department")
#         authorized_by = st.text_input("Authorized By", "CM INFOTECH")

# with tab4:
#     st.header("Preview & Generate")
#     total_base = sum(p["basic"] * p["qty"] for p in st.session_state.products)
#     total_gst = sum(p["basic"] * p["gst_percent"] / 100 * p["qty"] for p in st.session_state.products)
#     grand_total = total_base + total_gst
#     amount_words = num2words(grand_total, to="currency", currency="INR").title()
#     st.metric("Grand Total", f"₹{grand_total:,.2f}")

#     if st.button("Generate Document", type="primary"):
#         pdf = PDF()
#         if logo_file:
#             pdf.logo_path = logo_path
#         pdf.add_page()

#         # --- Vendor Info ---
#         pdf.section_title("Vendor & Addresses")
#         pdf.set_font("Calibri", "", 10)
#         pdf.multi_cell(95,5,f"{vendor_name}\n{vendor_address}\nAttn: {vendor_contact}\nMobile: {vendor_mobile}")
#         pdf.set_xy(110, pdf.get_y()-20)
#         pdf.multi_cell(90,5,f"Bill: {bill_to_company}\n{bill_to_address}\nShip: {ship_to_company}\n{ship_to_address}")
#         pdf.ln(1)
#         pdf.multi_cell(0,5,f"GST: {gst_no}\nPAN: {pan_no}\nMSME: {msme_no}")
#         pdf.ln(2)

#         # --- Products Table ---
#         if pdf_type=="Tax Invoice":
#             col_widths = [50,20,20,20,20,15,25]
#             headers = ["Product","Basic","CGST 9%","SGST 9%","Total GST","Qty","Total"]
#         else:
#             col_widths = [65,22,25,25,15,22]
#             headers = ["Product","Basic","GST Amount","Per Unit Price","Qty","Total"]

#         pdf.set_fill_color(220,220,220)
#         pdf.set_font("Calibri","B",10)
#         for h,w in zip(headers,col_widths):
#             pdf.cell(w,6,h,border=1,align="C",fill=True)
#         pdf.ln()

#         pdf.set_font("Calibri","",10)
#         line_height=5
#         grand_total=0
#         total_cgst=0
#         total_sgst=0
#         for p in st.session_state.products:
#             qty=p["qty"]; basic=p["basic"]; gst_percent=p["gst_percent"]
#             total_gst_per_unit = basic*gst_percent/100
#             total_amount = (basic+total_gst_per_unit)*qty
#             grand_total += total_amount
#             if pdf_type=="Tax Invoice":
#                 cgst=total_gst_per_unit/2; sgst=total_gst_per_unit/2
#                 total_cgst+=cgst*qty; total_sgst+=sgst*qty

#             x_start, y_start = pdf.get_x(), pdf.get_y()
#             pdf.multi_cell(col_widths[0], line_height, p["name"], border=1)
#             pdf.set_xy(x_start+col_widths[0], y_start)
#             if pdf_type=="Tax Invoice":
#                 pdf.cell(col_widths[1], line_height, f"{basic:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[2], line_height, f"{cgst:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[3], line_height, f"{sgst:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[4], line_height, f"{total_gst_per_unit:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[5], line_height, f"{qty:.2f}", border=1, align="C")
#                 pdf.cell(col_widths[6], line_height, f"{total_amount:.2f}", border=1, align="R")
#             else:
#                 pdf.cell(col_widths[1], line_height, f"{basic:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[2], line_height, f"{total_gst_per_unit:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[3], line_height, f"{basic+total_gst_per_unit:.2f}", border=1, align="R")
#                 pdf.cell(col_widths[4], line_height, f"{qty:.2f}", border=1, align="C")
#                 pdf.cell(col_widths[5], line_height, f"{total_amount:.2f}", border=1, align="R")
#             pdf.ln(line_height)

#         # --- Totals ---
#         pdf.set_font("Calibri","B",10)
#         if pdf_type=="Tax Invoice":
#             pdf.cell(col_widths[0]+col_widths[1],6,"Total",border=1,align="R")
#             pdf.cell(col_widths[2],6,f"{total_cgst:.2f}",border=1,align="R")
#             pdf.cell(col_widths[3],6,f"{total_sgst:.2f}",border=1,align="R")
#             pdf.cell(col_widths[4],6,f"{total_cgst+total_sgst:.2f}",border=1,align="R")
#             pdf.cell(col_widths[5],6,"",border=1)
#             pdf.cell(col_widths[6],6,f"{grand_total:.2f}",border=1,align="R")
#         else:
#             pdf.cell(sum(col_widths[:-1]),6,"Grand Total",border=1,align="R")
#             pdf.cell(col_widths[-1],6,f"{grand_total:.2f}",border=1,align="R")
#         pdf.ln(8)

#         # Amount in Words
#         pdf.set_font("Calibri","B",10)
#         pdf.cell(0,5,"Amount in Words:",ln=True)
#         pdf.set_font("Calibri","",10)
#         pdf.multi_cell(0,5,num2words(grand_total,to="currency",currency="INR").title())
#         pdf.ln(4)

#         # Terms & Authorization
#         pdf.section_title("Terms & Conditions")
#         pdf.set_font("Calibri","",10)
#         pdf.multi_cell(0,4,f"Taxes: As specified above\nPayment: {payment_terms}\nDelivery: {delivery_terms}")
#         pdf.ln(2)
#         pdf.cell(0,5,f"Prepared By: {prepared_by}",ln=1)
#         pdf.cell(0,5,f"Authorized By: {authorized_by}",ln=1)

#         # Footer Company Name
#         pdf.ln(5)
#         pdf.cell(0,5,f"For, {st.session_state.company_name}",ln=True,align="L")

#         pdf_bytes = pdf.output(dest="S").encode('latin-1')
#         if auto_increment:
#             st.session_state.po_seq +=1

#         st.success(f"{pdf_type} generated!")
#         st.download_button("⬇ Download Document", pdf_bytes, f"{pdf_type.replace(' ','_')}_{st.session_state.po_number.replace('/','_')}.pdf","application/pdf")






import streamlit as st
from fpdf import FPDF
from num2words import num2words
import datetime
import os

# --- Product Catalog ---
PRODUCT_CATALOG = {
    "Autodesk Commercial Software License": {"basic": 2000.0, "gst_percent": 18.0},
    "Solidworks Premium": {"basic": 50000.0, "gst_percent": 18.0},
    "Catia License": {"basic": 75000.0, "gst_percent": 18.0},
    "Mastercam Module": {"basic": 30000.0, "gst_percent": 18.0},
    "Siemens NX": {"basic": 65000.0, "gst_percent": 18.0},
}

# --- PDF Class ---
class PDF(FPDF):
    def __init__(self, doc_type="PO"):
        super().__init__()
        self.doc_type = doc_type  # "PO" or "Tax Invoice"
        self.set_auto_page_break(auto=False, margin=0)
        self.set_left_margin(15)
        self.set_right_margin(15)
        self.logo_path = None
        font_dir = os.path.join(os.path.dirname(__file__), "fonts")
        self.add_font("Calibri", "", os.path.join(font_dir, "calibri.ttf"), uni=True)
        self.add_font("Calibri", "B", os.path.join(font_dir, "calibrib.ttf"), uni=True)
        self.add_font("Calibri", "I", os.path.join(font_dir, "calibrii.ttf"), uni=True)
        self.add_font("Calibri", "BI", os.path.join(font_dir, "calibriz.ttf"), uni=True)
        self.website_url = "https://cminfotech.com/"

    def header(self):
        if self.page_no() == 1:
            if self.logo_path and os.path.exists(self.logo_path):
                self.image(self.logo_path, x=162.5, y=2.5, w=45, link=self.website_url)
            self.set_font("Calibri", "B", 15)
            title = "PURCHASE ORDER" if self.doc_type == "PO" else "TAX INVOICE"
            self.cell(0, 15, title, ln=True, align="C")
            self.ln(2)
            self.set_font("Calibri", "", 12)
            if hasattr(st.session_state, 'po_number'):
                self.cell(95, 8, f"PO No: {st.session_state.po_number}", ln=0)
            if hasattr(st.session_state, 'po_date'):
                self.cell(95, 8, f"Date: {st.session_state.po_date}", ln=1)
            self.ln(2)

    def footer(self):
        self.set_y(-18)
        self.set_font("Calibri", "I", 10)
        self.multi_cell(0, 4,
            "E402, Ganesh Glory 11, Near BSNL Office, Jagatpur - Chenpur Road, Ahmedabad - 382481\n",
            align="C"
        )
        self.set_text_color(0, 0, 255)
        email1 = "cad@cmi.com"
        email2 = "info@cminfotech.com"
        self.cell(0, 4, f"{email1} | {email2}", ln=True, align="C", link=f"mailto:{email1}")
        self.set_x((self.w - 80) / 2)
        self.cell(0, 0, "", link=f"mailto:{email2}")
        self.set_x((self.w - 60) / 2)
        phone_number = "+91 873 391 5721"
        self.cell(60, 4, f"Call: {phone_number}", ln=True, align="C", link=f"tel:{phone_number}")
        self.set_text_color(0, 0, 0)

    def section_title(self, title):
        self.set_font("Calibri", "B", 12)
        self.cell(0, 6, title, ln=True)
        self.ln(1)

    def sanitize_text(self, text):
        return text.encode('ascii', 'ignore').decode('ascii')

# --- Streamlit Setup ---
st.set_page_config(page_title="PO & Tax Invoice Generator", page_icon="📄", layout="wide")
st.title("📄 Purchase Order & Tax Invoice Generator")

# --- Session State ---
if "po_seq" not in st.session_state: st.session_state.po_seq = 1
if "products" not in st.session_state: st.session_state.products = []
if "company_name" not in st.session_state: st.session_state.company_name = "CM Infotech"

today = datetime.date.today()
st.session_state.po_date = today.strftime("%d-%m-%Y")
default_po_number = f"C/CP/{today.year}/Q{(today.month-1)//3+1}/{st.session_state.po_seq:03d}"
st.session_state.po_number = st.text_input("PO Number", value=default_po_number)

# --- Sidebar ---
with st.sidebar:
    st.header("Company Info")
    logo_file = st.file_uploader("Upload Company Logo", type=["png", "jpg", "jpeg"])
    auto_increment = st.checkbox("Auto-increment PO Number", value=True)
    if st.button("Reset PO Sequence"): st.session_state.po_seq = 1; st.success("PO sequence reset to 1")

# --- Tabs ---
tab1, tab2, tab3, tab4 = st.tabs(["Vendor Details", "Products", "Terms", "Preview & Generate"])

# Vendor Details
with tab1:
    col1, col2 = st.columns(2)
    with col1:
        vendor_name = st.text_input("Vendor Name", "Arkance IN Pvt. Ltd.")
        vendor_address = st.text_area("Vendor Address", "Unit 801-802, 8th Floor, Tower 1...")
        vendor_contact = st.text_input("Contact Person", "Ms/Mr")
        vendor_mobile = st.text_input("Mobile", "+91 1234567890")
        gst_no = st.text_input("GST No", "24ANMPP4891R1ZX")
        pan_no = st.text_input("PAN No", "ANMPP4891R")
        msme_no = st.text_input("MSME No", "UDYAM-GJ-01-0117646")
    with col2:
        bill_to_company = st.text_input("Bill To", "CM INFOTECH")
        bill_to_address = st.text_area("Bill To Address", "E/402, Ganesh Glory 11, ...")
        ship_to_company = st.text_input("Ship To", "CM INFOTECH")
        ship_to_address = st.text_area("Ship To Address", "E/402, Ganesh Glory 11, ...")

# Products
with tab2:
    st.header("Products")
    selected_product = st.selectbox("Select from Catalog", [""] + list(PRODUCT_CATALOG.keys()))
    if st.button("➕ Add Selected Product") and selected_product:
        details = PRODUCT_CATALOG[selected_product]
        st.session_state.products.append({"name": selected_product, "basic": details["basic"], "gst_percent": details["gst_percent"], "qty": 1.0})
    if st.button("➕ Add Empty Product"): st.session_state.products.append({"name": "New Product", "basic": 0.0, "gst_percent": 18.0, "qty": 1.0})
    for i, p in enumerate(st.session_state.products):
        with st.expander(f"Product {i+1}: {p['name']}", expanded=i==0):
            cols = st.columns([3,1,1,1])
            st.session_state.products[i]["name"] = st.text_input("Name", p["name"], key=f"name_{i}")
            st.session_state.products[i]["basic"] = st.number_input("Basic (₹)", p["basic"], format="%.2f", key=f"basic_{i}")
            st.session_state.products[i]["gst_percent"] = st.number_input("GST %", p["gst_percent"], format="%.1f", key=f"gst_{i}")
            st.session_state.products[i]["qty"] = st.number_input("Qty", p["qty"], format="%.2f", key=f"qty_{i}")
            if st.button("Remove", key=f"remove_{i}"): st.session_state.products.pop(i); st.rerun()

# Terms
with tab3:
    st.header("Terms & Authorization")
    payment_terms = st.text_input("Payment Terms", "30 Days from Invoice date")
    delivery_days = st.number_input("Delivery (Days)", min_value=1, value=2)
    delivery_terms = st.text_input("Delivery Terms", f"Within {delivery_days} Days")
    prepared_by = st.text_input("Prepared By", "Finance Department")
    authorized_by = st.text_input("Authorized By", "CM INFOTECH")

# Preview & Generate
with tab4:
    st.header("Preview & Generate")
    total_base = sum(p["basic"]*p["qty"] for p in st.session_state.products)
    total_gst = sum(p["basic"]*p["gst_percent"]/100*p["qty"] for p in st.session_state.products)
    grand_total = total_base + total_gst
    amount_words = num2words(grand_total, to="currency", currency="INR").title()
    st.metric("Grand Total", f"₹{grand_total:,.2f}")

    if st.button("Generate PO & Tax Invoice"):
        for doc_type in ["PO", "Tax Invoice"]:
            pdf = PDF(doc_type=doc_type)
            if logo_file:
                pdf.logo_path = logo_file.name
            pdf.add_page()
            # --- Vendor & Addresses ---
            pdf.section_title("Vendor & Addresses")
            pdf.set_font("Calibri", "", 10)
            pdf.multi_cell(95, 5, f"{vendor_name}\n{vendor_address}\nAttn: {vendor_contact}\nMobile: {vendor_mobile}")
            pdf.set_xy(110, pdf.get_y() - 20)
            pdf.multi_cell(90,5,f"Bill: {bill_to_company}\n{bill_to_address}\nShip: {ship_to_company}\n{ship_to_address}")
            pdf.ln(2)
            # --- Products Table Function ---
            pdf.section_title("Products & Services")
            col_widths = [65,22,25,25,15,22]
            headers = ["Product","Basic","GST Amt","Per Unit Price","Qty","Total"]
            pdf.set_fill_color(220,220,220)
            pdf.set_font("Calibri","B",10)
            for h,w in zip(headers,col_widths): pdf.cell(w,6,h,border=1,align="C",fill=True)
            pdf.ln()
            pdf.set_font("Calibri","",10)
            line_height=5
            for p in st.session_state.products:
                gst_amt=p["basic"]*p["gst_percent"]/100
                per_unit_price=p["basic"]+gst_amt
                total=(p["basic"]+gst_amt)*p["qty"]
                name = pdf.sanitize_text(p["name"])
                num_lines = pdf.multi_cell(col_widths[0], line_height, name, border=0, split_only=True)
                row_height = max(len(num_lines),1)*line_height
                x_start = pdf.get_x()
                y_start = pdf.get_y()
                pdf.multi_cell(col_widths[0], line_height, name, border=1)
                pdf.set_xy(x_start+col_widths[0],y_start)
                pdf.cell(col_widths[1],row_height,f"{p['basic']:.2f}",border=1,align="R")
                pdf.cell(col_widths[2],row_height,f"{gst_amt:.2f}",border=1,align="R")
                pdf.cell(col_widths[3],row_height,f"{per_unit_price:.2f}",border=1,align="R")
                pdf.cell(col_widths[4],row_height,f"{p['qty']:.2f}",border=1,align="C")
                pdf.cell(col_widths[5],row_height,f"{total:.2f}",border=1,align="R")
                pdf.ln(row_height)
            # Grand Total
            pdf.set_font("Calibri","B",10)
            pdf.cell(sum(col_widths[:-1]),6,"Grand Total",border=1,align="R")
            pdf.cell(col_widths[5],6,f"{grand_total:.2f}",border=1,align="R")
            pdf.ln(4)
            # Amount in words
            pdf.ln(2)
            pdf.set_font("Calibri","B",10)
            pdf.cell(0,5,"Amount in Words:",ln=True)
            pdf.set_font("Calibri","",10)
            pdf.multi_cell(0,5,amount_words)
            # Authorization
            pdf.ln(2)
            pdf.set_x(pdf.l_margin)
            pdf.cell(0,5,f"Prepared By: {prepared_by}",ln=1)
            pdf.set_x(pdf.l_margin)
            pdf.cell(0,5,f"Authorized By: {authorized_by}",ln=1)
            # Footer company name + stamp
            pdf.ln(2)
            pdf.cell(0,5,f"For, {st.session_state.company_name}",ln=True,align="L")
            stamp_path = os.path.join(os.path.dirname(__file__),"stamp.jpg")
            if os.path.exists(stamp_path): pdf.image(stamp_path,x=pdf.get_x(),y=pdf.get_y(),w=30)
            pdf_bytes = pdf.output(dest="S").encode('latin-1')
            st.download_button(f"⬇ Download {doc_type}", pdf_bytes, f"{doc_type}_{st.session_state.po_number.replace('/', '_')}.pdf","application/pdf")
        if auto_increment: st.session_state.po_seq += 1