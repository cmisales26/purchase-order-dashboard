...
def create_quotation_pdf(quotation_data, logo_path = "logo_final.jpg"):
    ...
    # --- Recipient ---
    pdf.section_title("Recipient")
    pdf.set_font("Helvetica", "", 10)
    pdf.multi_cell(0, 5, f"To: {pdf.sanitize_text(data['vendor_name'])}\nAddress: {pdf.sanitize_text(data['vendor_address'])}\nKind Attention: {pdf.sanitize_text(data['vendor_contact'])}")
    pdf.ln(5)

    # --- Products Table (Annexure I - Commercials) ---
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 6, "Annexure I - Commercials: Quotation for Adobe Software", ln=True)
    pdf.ln(1)

    # Column widths & headers matching Quotation_DEMO.pdf Annexure I [cite: 76]
    col_widths = [65, 30, 30, 30, 15, 25] 
    ...
    for p in data["products"]:
        ...
        # Calculations (GST amount per unit for display in column 3)
        gst_amt_per_unit = basic_price * gst_percent / 100
        per_unit_price = basic_price + gst_amt_per_unit
        total = per_unit_price * qty
        total_amount += total

        # Displaying data row
        pdf.cell(col_widths[0], 6, pdf.sanitize_text(p["name"]), border=1)
        pdf.cell(col_widths[1], 6, f"{basic_price:,.2f}", border=1, align="R")
        pdf.cell(col_widths[2], 6, f"{gst_amt_per_unit:,.2f}", border=1, align="R") 
        pdf.cell(col_widths[3], 6, f"{per_unit_price:,.2f}", border=1, align="R")
        pdf.cell(col_widths[4], 6, f"{qty:,.2f}", border=1, align="C")
    ...
    pdf.cell(sum(col_widths[:-1]), 6, "Grand Total", border=1, align="R")
    pdf.cell(col_widths[-1], 6, f"{total_amount:,.2f}", border=1, align="R")
    pdf.ln(4)

    # --- Amount in Words ---
    pdf.set_font("Helvetica", "B", 10)
    pdf.multi_cell(0, 5, f"Amount in Words: {pdf.sanitize_text(data['amount_words'])}")
    pdf.ln(5)

    # --- Terms & Conditions (Itemized, matching demo layout) ---
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 6, "Terms and Conditions:", ln=True)
    pdf.set_font("Helvetica", "", 9)

    terms = [
        "Above charges are Inclusive of GST[cite: 80].",
        "Any changes in Govt. duties, Taxes & Forex rate at the time of dispatch shall be applicable[cite: 81, 82].",
        "TDS should not be deducted at the time of payment as per Govt. NOTIFICATION NO. 21/2012 [F.No.142/10/2012-SO (TPL)] S.O. 1323(E), DATED 13-6-2012[cite: 83, 84].",
        "ELD licenses are paper licenses that do not contain media[cite: 85, 86].",
        "An Internet connection is required to access cloud services[cite: 87].",
        "Training will be charged at extra cost depending on no. of participants[cite: 89, 90].",
        f"Price Validity: {pdf.sanitize_text(data['price_validity'])}[cite: 92].",
        "Payment: 100% Advance along with purchase order[cite: 93].",
        "Delivery period: 1-2 Weeks from the date of Purchase Order[cite: 94].",
        'Cheque to be issued on name of: "CM INFOTECH"[cite: 96].',
        "Order to be placed on: CM INFOTECH[cite: 97].",
    ]
    ...
    # --- Signature Block ---
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(0, 5, "Yours Truly,", ln=1)
    pdf.cell(0, 5, "For CM INFOTECH", ln=1)
    pdf.ln(10) # Space for signature/stamp
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 5, "Chirag Darji", ln=1)
    pdf.cell(0, 5, "Inside Sales Executive", ln=1)

    return pdf.output(dest="S").encode('latin-1')

# --- Example Data (Derived from Quotation_DEMO.pdf) ---
total_basic_ex = 114560.00 + 104560.00 
grand_total_ex = 258561.60

example_quotation_data = {
    "quotation_number": "Q/2025-2026/004",
    "quotation_date": "29-09-2025",
    "vendor_name": "Creation Studio",
    "vendor_address": "Al-Habtula Apartment, Swk Society, Sid, Dah, Guja 389",
    "vendor_contact": "Mr. Musta",
    "products": [
        {"name": "Creative cloud pro plus for Teams", "basic": 114560.00, "qty": 1.0},
        {"name": "Creative cloud Pro for Teams", "basic": 104560.00, "qty": 1.0},
    ],
    "price_validity": "September 29, 2025",
    "grand_total": grand_total_ex,
    "amount_words": num2words(grand_total_ex, to='currency', currency='INR').replace("INR", "Rs.").title(),
}

# Generate and store the PDF bytes
pdf_bytes = create_quotation_pdf(example_quotation_data)

# Use the file tool to make the content accessible
io.BytesIO(pdf_bytes)

print("The revised 'Quotation' PDF is ready. Please download it below. The commercial table and terms now match your demo.")
