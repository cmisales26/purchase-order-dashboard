...
# --- Function to update totals and amount in words ---
def update_totals(data):
    ...
    amount_words = "Rs. " + num2words(int(final_amount), to='currency').replace("euro", "Rupees").replace("cents", "Paise") + " Only/-"

    data["totals"]["basic_amount"] = round(total_basic,2)
    data["totals"]["sgst"] = round(sgst,2)
    data["totals"]["cgst"] = round(cgst,2)
    data["totals"]["final_amount"] = round(final_amount,2)
    data["totals"]["amount_in_words"] = amount_words
...
def create_invoice_pdf(data):
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(100, 5, data["supplier"]["name"], border='L,R', ln=1)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_x(15)
    pdf.multi_cell(100, 5, data["supplier"]["address"], border=0)
    pdf.set_x(15)
    pdf.cell(100, 5, f"GST No.: {data['supplier']['gst_no']}", border='L,R', ln=1)
    pdf.set_x(15)
    pdf.cell(100, 5, f"MSME Reg No.: {data['supplier']['msme_reg_no']}", border='L,R', ln=1)
    pdf.set_x(15)
    pdf.cell(100, 5, f"E-Mail: {data['supplier']['email']}", border='L,R', ln=1)
    pdf.set_x(15)
    pdf.cell(100, 5, f"Mobile No.: {data['supplier']['mobile_no']}", border='L,R,B', ln=1)

    # Buyer Details
    pdf.set_xy(115, y_start + 5)
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(95, 5, data["buyer"]["name"], border='L,R', ln=1)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_x(115)
    pdf.multi_cell(95, 5, data["buyer"]["address"], border=0)
    pdf.set_x(115)
    pdf.cell(95, 5, f"GST No.: {data['buyer']['gst_no']}", border='L,R', ln=1)
    pdf.set_x(115)
    pdf.cell(95, 5, f"Email: {data['buyer']['email']}", border='L,R', ln=1)
    pdf.set_x(115)
    pdf.cell(95, 5, f"Tel No.: {data['buyer']['tel_no']}", border='L,R', ln=1)
    pdf.set_x(115)
    pdf.cell(95, 5, f"Destination: {data['buyer']['destination']}", border='L,R,B', ln=1)
    ...
    pdf.cell(col_widths[1], 7, data["invoice"]["invoice_no"], border=1)
    pdf.cell(col_widths[2], 7, "Invoice Date", border=1)
    pdf.cell(col_widths[3], 7, data["invoice"]["invoice_date"], border=1, ln=1)
    pdf.cell(col_widths[0], 7, "Buyer's Order No.", border=1)
    pdf.cell(col_widths[1], 7, data["invoice"]["buyers_order_no"], border=1)
    pdf.cell(col_widths[2], 7, "Buyer's Order Date", border=1)
    pdf.cell(col_widths[3], 7, data["invoice"]["buyers_order_date"], border=1, ln=1)
    pdf.cell(col_widths[0], 7, "Terms of Payment", border=1)
    pdf.cell(col_widths[1], 7, data["invoice"]["terms_of_payment"], border=1)
    pdf.cell(col_widths[2], 7, "Dispatched Through", border=1)
    pdf.cell(col_widths[3], 7, data["invoice"]["dispatched_through"], border=1, ln=1)
    pdf.ln(5)
    ...
    for i, item in enumerate(data["items"]):
        ...
        pdf.cell(col_widths[0],7,str(i+1),border=1,align='C')
        pdf.cell(col_widths[1],7,item["description"],border=1)
        pdf.cell(col_widths[2],7,item["hsn_sac"],border=1,align='C')
        pdf.cell(col_widths[3],7,f"{item['quantity']:.2f}",border=1,align='R')
        pdf.cell(col_widths[4],7,f"{item['unit_rate']:.2f}",border=1,align='R')
    ...
    pdf.cell(col_widths[-1],7,f"{data['totals']['basic_amount']:.2f}",border=1,align='R',ln=1)
    pdf.cell(sum(col_widths[:-1]),7,"SGST @ 9%",border=1,align='R')
    pdf.cell(col_widths[-1],7,f"{data['totals']['sgst']:.2f}",border=1,align='R',ln=1)
    pdf.cell(sum(col_widths[:-1]),7,"CGST @ 9%",border=1,align='R')
    pdf.cell(col_widths[-1],7,f"{data['totals']['cgst']:.2f}",border=1,align='R',ln=1)
    pdf.cell(sum(col_widths[:-1]),7,"Final Amount to be Paid",border=1,align='R')
    pdf.cell(col_widths[-1],7,f"{data['totals']['final_amount']:.2f}",border=1,align='R',ln=1)
    pdf.ln(5)
    ...
    pdf.set_font("Helvetica","",10)
    pdf.multi_cell(0,7,data["totals"]["amount_in_words"])
    pdf.ln(5)
    ...
    pdf.set_font("Helvetica","",10)
    pdf.cell(0,7,data["bank_details"]["bank_name"],border='R',ln=1)
    pdf.set_font("Helvetica","B",10)
    pdf.cell(50,7,"Branch:",border='L',ln=0)
    pdf.set_font("Helvetica","",10)
    pdf.cell(0,7,data["bank_details"]["branch"],border='R',ln=1)
    pdf.set_font("Helvetica","B",10)
    pdf.cell(50,7,"Account No.:",border='L',ln=0)
    pdf.set_font("Helvetica","",10)
    pdf.cell(0,7,data["bank_details"]["account_no"],border='R',ln=1)
    pdf.set_font("Helvetica","B",10)
    pdf.cell(50,7,"IFS Code:",border='L,B',ln=0)
    pdf.set_font("Helvetica","",10)
    pdf.cell(0,7,data["bank_details"]["ifs_code"],border='R,B',ln=1)
    pdf.ln(5)

    # Declaration & Signature
    pdf.set_font("Helvetica","",8)
    pdf.multi_cell(0,4,data["declaration"])
    pdf.ln(5)
    pdf.set_font("Helvetica","",10)
...
if "invoice_data" not in st.session_state:
    st.session_state.invoice_data = {
        "supplier":{"name":"CM Infotech","address":"E/402, Ganesh Glory 11, Near BSNL Office, Jagatpur, Chenpur Road, Jagatpur Village, Ahmedabad - 382481","gst_no":"24ANMPP4","msme_reg_no":"UDYAM-","email":"cm.infot","mobile_no":"873391"},
        "buyer":{"name":"Baldridge Pvt Ltd.","address":"406, Sakar East, 40mt Tarsali - Danteshwar Ring Road, Vadodara 390009","email":"dmistry@b","tel_no":"98987","gst_no":"24AAHCB9","destination":"Vadodara"},
        "invoice":{"invoice_no":"CMI/25-26/Q1/010","invoice_date":"28 April 2025","buyers_order_no":"","buyers_order_date":"17 April 2025","terms_of_payment":"100% Advance with Purchase Order","dispatched_through":"Online","terms_of_delivery":"Within Month"},
        "items":[{"description":"Autodesk BIM Collaborate Pro - Single-user CLOUD Commercial New Annual Subscription","hsn_sac":"997331","quantity":1.00,"unit_rate":36500.00}],
        "totals":{"basic_amount":0.0,"sgst":0.0,"cgst":0.0,"final_amount":0.0,"amount_in_words":""},
        "bank_details":{"bank_name":"XYZ bank","branch":"AHMED","account_no":"881304","ifs_code":"IDFB004"},
        "declaration":"IT IS HEREBY DECLARED THAT THE SOFTWARE HAS ALREADY BEEN DEDUCTED FOR TDS/WITH HOLDING TAX..."
    }

st.subheader("Invoice Details")
col1, col2 = st.columns(2)
with col1:
    st.session_state.invoice_data["invoice"]["invoice_no"] = st.text_input("Invoice No.", st.session_state.invoice_data["invoice"]["invoice_no"])
with col2:
    st.session_state.invoice_data["invoice"]["invoice_date"] = st.text_input("Invoice Date", st.session_state.invoice_data["invoice"]["invoice_date"])

st.subheader("Products")
if st.button("➕ Add New Product"):
    st.session_state.invoice_data["items"].append({"description":"New Product","hsn_sac":"","quantity":1.0,"unit_rate":0.0})
...
if st.button("Generate Tax Invoice"):
    update_totals(st.session_state.invoice_data)
    pdf_file = create_invoice_pdf(st.session_state.invoice_data)
    st.success("Tax Invoice generated!")
    st.download_button("⬇ Download Tax Invoice", pdf_file.getvalue(), "tax_invoice.pdf", "application/pdf")
