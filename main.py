...
class PDF(FPDF):
                ...
                try:
                    with Image.open(self.logo_path) as img:
                        if img.format == "PNG" and img.info.get("interlace"):
                            ...
                            img.save(temp_path, interlace=0)
                            self.image(temp_path, x=163, y=8, w=45)
                        else:
                            self.image(self.logo_path, x=163, y=8, w=45)
                except Exception:
                    pass
    ...
    def footer(self):
        ...
        self.set_text_color(0, 0, 255)
        email1, email2 = "cad@cmi.com", "support@cmi.com"
        self.cell(0, 4, f"{email1} | {email2}", ln=True, align="C", link=f"mailto:{email1}")
        self.set_x((self.w - 60) / 2)
        phone_number = "+918798159721"
        self.cell(60, 4, f"Call: {phone_number}", ln=True, align="C", link=f"tel:{phone_number}")
...
# --- Tabs ---
tab1, tab2, tab3, tab4 = st.tabs(["Vendor Details", "Products", "Terms", "Authorization & Generate"])
...
# --- Tab 4: Authorization & Generate ---
with tab4:
    st.header("Authorization & Generate")
    col1, col2 = st.columns(2)
    with col1:
        prepared_by = st.text_input("Prepared By", "Finance Department")
    with col2:
    ...
    if st.button("Generate PO", type="primary"):
        ...
        pdf.section_title("Vendor & Addresses")
        pdf.set_font("Arial", "", 10)
        pdf.multi_cell(95, 5, f"{pdf.sanitize_text(vendor_name)}\n{pdf.sanitize_text(vendor_address)}\nAttn: {pdf.sanitize_text(vendor_contact)}\nMobile: {pdf.sanitize_text(vendor_mobile)}")
        pdf.set_xy(110, pdf.get_y() - 20)
        pdf.multi_cell(90, 5, f"Bill: {pdf.sanitize_text(bill_to_company)}\n{pdf.sanitize_text(bill_to_address)}\nShip: {pdf.sanitize_text(ship_to_company)}\n{pdf.sanitize_text(ship_to_address)}")
        pdf.ln(1)
        pdf.multi_cell(0, 5, f"GST: {pdf.sanitize_text(gst_no)}\nPAN: {pdf.sanitize_text(pan_no)}\nMSME: {pdf.sanitize_text(msme_no)}")
        pdf.ln(2)
        ...
        pdf.section_title("Terms & Conditions")
        pdf.set_font("Arial", "", 10)
        pdf.multi_cell(0, 4, f"Payment: {pdf.sanitize_text(payment_terms)}\nDelivery: {pdf.sanitize_text(delivery_terms)}")
        pdf.ln(2)

        # --- End User ---
        pdf.section_title("End User Details")
        pdf.set_font("Arial", "", 10)
        pdf.multi_cell(0, 4, f"{pdf.sanitize_text(end_company)}\n{pdf.sanitize_text(end_address)}\nContact: {pdf.sanitize_text(end_person)} | {pdf.sanitize_text(end_contact)}\nEmail: {pdf.sanitize_text(end_email)}")
        pdf.ln(2)

        # --- Authorization ---
        pdf.set_font("Arial", "", 10)
        pdf.cell(0, 5, f"Prepared By: {pdf.sanitize_text(prepared_by)}", ln=1, border=0)
        pdf.cell(0, 5, f"Authorized By: {pdf.sanitize_text(authorized_by)}", ln=1, border=0)
            ...
            try:
                with Image.open(stamp_path) as img:
                    if img.format == "PNG" and img.info.get("interlace"):
                        ...
                        img.save(temp_stamp, interlace=0)
                        pdf.set_y(-105)
                        pdf.set_x(15)
                        pdf.image(temp_stamp, w=30)
                    else:
                        pdf.set_y(-105)
                        pdf.set_x(15)
