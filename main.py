...
class PDF(FPDF):
    ...
    def _safe_image(self, path, x=None, y=None, w=0, h=0):
        """Ensures the image is non-interlaced/compatible before inserting into PDF."""
        try:
            img = Image.open(path)
            if img.format == "PNG":
                fixed_path = path.replace(".png", "_fixed.png")
                img.save(fixed_path, interlace=False)
                path = fixed_path
            elif img.format not in ["JPEG", "JPG", "PNG"]:
                fixed_path = os.path.splitext(path)[0] + "_fixed.jpg"
                img.convert("RGB").save(fixed_path, "JPEG")
                path = fixed_path
    ...
    def header(self):
        if self.logo_path:
            self._safe_image(self.logo_path, x=163, y=8, w=45)
        self.set_font("Arial", "B", 14)
        self.cell(0, 10, "PURCHASE ORDER", ln=1, align="C")
        self.ln(5)

    def add_stamp(self):
        """Adds stamp just above Authorized By section."""
        if self.stamp_path:
            y_pos = self.get_y() + 5
            self._safe_image(self.stamp_path, x=150, y=y_pos, w=30)
...
with st.sidebar:
    ...
    stamp_file = st.file_uploader("Upload Stamp/Signature", type=["png", "jpg", "jpeg"])

# PO details
today = datetime.date.today().strftime("%d-%m-%Y")
po_number = st.text_input("PO Number", f"PO-{today.replace('-', '')}")
po_date = st.text_input("PO Date", today)

vendor_name = st.text_input("Vendor Name", "Arkance IN Pvt. Ltd.")
vendor_address = st.text_area("Vendor Address", "Unit 801-802, Tower 1...")
bill_to = st.text_input("Bill To", "CM INFOTECH")
ship_to = st.text_input("Ship To", "CM INFOTECH")
...
for i in range(num_products):
    ...
    name = st.text_input(f"Name {i+1}", f"Product {i+1}")
    basic = st.number_input(f"Basic Price {i+1}", min_value=0.0, value=1000.0, step=100.0)
    gst_percent = st.number_input(f"GST % {i+1}", min_value=0.0, value=18.0, step=0.5)
    qty = st.number_input(f"Qty {i+1}", min_value=1.0, value=1.0, step=1.0)
    products.append({"name": name, "basic": basic, "gst_percent": gst_percent, "qty": qty})
...
end_user_name = st.text_input("End User Name", "ABC Pvt Ltd")
end_user_contact = st.text_input("Contact Person", "Mr. Sharma")
end_user_email = st.text_input("Email", "sharma@abc.com")
end_user_phone = st.text_input("Phone", "+91-9999999999")
...
if st.button("Generate PO"):
    ...
    pdf = PDF(logo_path=logo_path, stamp_path=stamp_path)
    pdf.add_page()

    # Vendor
    pdf.section_title("Vendor & Addresses")
    pdf.set_font("Arial", "", 10)
    pdf.multi_cell(0, 5, f"{pdf.sanitize(vendor_name)}\n{pdf.sanitize(vendor_address)}")
    pdf.ln(2)
    pdf.multi_cell(0, 5, f"Bill To: {pdf.sanitize(bill_to)}\nShip To: {pdf.sanitize(ship_to)}")
    pdf.ln(5)

    # Products Table
    pdf.section_title("Products & Services")
    col_widths = [65, 25, 25, 20, 25]
    headers = ["Product", "Basic", "GST", "Qty", "Total"]
    pdf.set_fill_color(220, 220, 220)
    pdf.set_font("Arial", "B", 10)
    for h, w in zip(headers, col_widths):
        pdf.cell(w, 7, pdf.sanitize(h), border=1, align="C", fill=True)
    pdf.ln()
    pdf.set_font("Arial", "", 10)

    total_base, total_gst, grand_total = 0, 0, 0
    for p in products:
        gst_amt = p["basic"] * p["gst_percent"] / 100
        total = (p["basic"] + gst_amt) * p["qty"]
        total_base += p["basic"] * p["qty"]
        total_gst += gst_amt * p["qty"]
        grand_total += total
        pdf.cell(col_widths[0], 6, pdf.sanitize(p["name"]), border=1)
        pdf.cell(col_widths[1], 6, f"{p['basic']:.2f}", border=1, align="R")
        pdf.cell(col_widths[2], 6, f"{gst_amt:.2f}", border=1, align="R")
        pdf.cell(col_widths[3], 6, f"{p['qty']:.0f}", border=1, align="C")
        pdf.cell(col_widths[4], 6, f"{total:.2f}", border=1, align="R")
        pdf.ln()
    ...
    pdf.set_font("Arial", "", 10)
    amount_words = num2words(grand_total, to="currency", currency="INR").title()
    pdf.multi_cell(0, 6, pdf.sanitize(amount_words))
    pdf.ln(5)

    # Terms
    pdf.section_title("Terms & Conditions")
    pdf.set_font("Arial", "", 10)
    pdf.multi_cell(0, 5, f"Taxes: As specified above\nPayment: {pdf.sanitize(payment_terms)}\nDelivery: {pdf.sanitize(delivery_terms)}")
    pdf.ln(5)

    # End User
    pdf.section_title("End User Details")
    pdf.set_font("Arial", "", 10)
    pdf.multi_cell(0, 5, f"Company: {pdf.sanitize(end_user_name)}\nContact: {pdf.sanitize(end_user_contact)}\nEmail: {pdf.sanitize(end_user_email)}\nPhone: {pdf.sanitize(end_user_phone)}")
    pdf.ln(10)
    ...
    pdf.add_stamp()
    pdf.ln(30)
    pdf.cell(0, 6, f"Prepared By: {pdf.sanitize(prepared_by)}", ln=1)
    pdf.cell(0, 6, f"Authorized By: {pdf.sanitize(authorized_by)}", ln=1, align="R")
